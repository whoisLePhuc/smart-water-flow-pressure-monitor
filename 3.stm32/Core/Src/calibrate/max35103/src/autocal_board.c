...
static void AUTOCAL_Log(const char *format, ...)
{
    ...
    va_end(args);
    if (length <= 0) return;
    const uint16_t txlen = (uint16_t)((length < (int)sizeof(buffer))
                   ...
                   const Max35103Profile *seed_profile)
{
    ...
    status = MAX35103_AutoCalDefaultConfig(&config, MAX35103_ACOUSTIC_PATH_UM, MAX35103_TRANSDUCER_FREQUENCY_HZ);
    ...
    config.dpl_min = 1U; config.dpl_max = 1U;
    config.ct_mask = 0x0FU;
    config.dly_min = 0x001CU; config.dly_max = 0x0023U;
    config.dly_coarse_step = 1U; config.dly_fine_step = 1U;
    ...
    status = MAX35103_AutoCalInit(&s_calibrator, &s_backend, &config, seed_profile,
                                   s_samples, MAX35103_AUTOCAL_SAMPLE_CAPACITY);
    if (status != MAX35103_AUTOCAL_OK) {
    ...
    AUTOCAL_Log(
        ...
        (unsigned)s_calibrator.config.dly_min, (unsigned)s_calibrator.config.dly_max,
        (unsigned)s_calibrator.config.dpl_min, (unsigned)s_calibrator.config.dpl_max,
    ...
    AUTOCAL_Log(
        "AUTOCAL|SEED|TOF1=%04X|TOF2=%04X|TOF3=%04X"
        "|TOF4=%04X|TOF5=%04X|TOF6=%04X|TOF7=%04X|DLY=%04X\r\n",
        (unsigned)seed_profile->tof1, (unsigned)seed_profile->tof2,
        (unsigned)seed_profile->tof3, (unsigned)seed_profile->tof4,
        (unsigned)seed_profile->tof5, (unsigned)seed_profile->tof6,
        (unsigned)seed_profile->tof7, (unsigned)seed_profile->tof_measurement_delay);
...
void AUTOCAL_Poll(void)
{
    ...
    /* DIAG_CFG — per-candidate config diagnostic */
    const bool new_evaluated =
    ...
    if (new_evaluated && (progress.state != MAX35103_AUTOCAL_STATE_DISCOVERY || diag_due)) {
        ...
        const uint32_t pe_ps = m->median_period_error_ps < 0 ? 0U
            : m->median_period_error_ps > (int64_t)UINT32_MAX
                ? UINT32_MAX : (uint32_t)m->median_period_error_ps;
        ...
        AUTOCAL_Log(
            ...
            (unsigned)dpl, (unsigned)pl, (unsigned)ct, (unsigned)pol,
        ...
        AUTOCAL_Log(
            ...
            (unsigned long)progress.evaluated_candidate_count,
            (unsigned)m->valid_count, (unsigned)m->attempted_count,
            (unsigned)m->physical_count, (unsigned)m->wave_valid_count,
            ...
            (long)up_ns, (long)dn_ns, (long)dd_ns,
            (unsigned long)pe_ps,
    ...
    if (fb_changed) {
        AUTOCAL_Log(
            "AUTOCAL|BACKTRACK|from=%s|to=%s|fallback=%u/%u|finalist=%u/%u\r\n",
    ...
    if (st_changed && !fb_changed &&
        ...
        AUTOCAL_Log(
            "AUTOCAL|STAGE_PASS|from=%s|to=%s|TOF1=%04X|DLY=%u\r\n",
            MAX35103_AutoCalStateName(s_last_logged_state),
            MAX35103_AutoCalStateName(progress.state),
            (unsigned)sel->tof1, (unsigned)sel->tof_measurement_delay);
    }
    ...
    if (status == MAX35103_AUTOCAL_COMPLETE) {
        if (MAX35103_AutoCalGetReport(&s_calibrator, &s_report) == MAX35103_AUTOCAL_COMPLETE) {
            ...
            AUTOCAL_Log(
                "AUTOCAL|PROFILE|TOF1=%04X|TOF2=%04X|TOF3=%04X"
                "|TOF4=%04X|TOF5=%04X|TOF6=%04X|TOF7=%04X|DLY=%04X\r\n",
                (unsigned)profile->tof1, (unsigned)profile->tof2,
                (unsigned)profile->tof3, (unsigned)profile->tof4,
                (unsigned)profile->tof5, (unsigned)profile->tof6,
                (unsigned)profile->tof7, (unsigned)profile->tof_measurement_delay);

            (void)MAX35103_Configure(s_driver,
                &s_report.selected_profile);
        ...
        const uint32_t fp_ps = f->median_period_error_ps < 0 ? 0U
            : f->median_period_error_ps > (int64_t)UINT32_MAX
                ? UINT32_MAX : (uint32_t)f->median_period_error_ps;
        AUTOCAL_Log(
            "AUTOCAL|FAIL|status=%d|state=%s|candidate=%lu|retry=%u|driver=%d"
            "|TOF1=%04X|DLY=%u\r\n",
            (int)status,
            ...
            (int)progress.last_driver_status,
            (unsigned)fp->tof1, (unsigned)fp->tof_measurement_delay);
        AUTOCAL_Log(
            ...
            (unsigned)f->valid_count, (unsigned)f->attempted_count,
            (unsigned)f->physical_count, (unsigned)f->wave_valid_count,
