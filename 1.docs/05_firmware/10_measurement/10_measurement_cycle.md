...
`MeasurementPurpose` map một-một tới provenance policy; không được chọn provenance lại ở downstream.
...
### 7.7. Event binding với Phase 1 catalog

| Event ID hiện có                 | Producer                     | Consumer/ý nghĩa                                 |
| -------------------------------- | ---------------------------- | ------------------------------------------------ |
| `EVT_MAX_RESULT_READY`           | MAX INT adapter/driver owner | Bắt đầu coherent read/validation                 |
...
| `EVT_PRESSURE_EOC`               | GPIO/driver adapter          | Pressure conversion completion evidence          |
...
| Purpose         | Required provenance                                                          | Production side effect                                         |
| --------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Boot self-check | `LIVE_PRODUCTION` hoặc dedicated self-check tag if added                     | Chỉ readiness; không volume/leak history                       |
| Production      | `LIVE_PRODUCTION`                                                            | Có thể admission sau full quality checks                       |
| Service         | `SERVICE_SAMPLE`                                                             | Cấm production volume/leak/scheduled telemetry                 |
| Calibration     | `CALIBRATION_SAMPLE`                                                         | Cấm production side effect                                     |
| Diagnostic      | `SERVICE_SAMPLE` hoặc dedicated diagnostic tag if added                      | Diagnostic only                                                |
| Recovery verify | `LIVE_PRODUCTION` chỉ khi production configuration/path được phục hồi đầy đủ | Readiness evidence; product update theo explicit resume policy |

Nếu cần provenance `SELF_CHECK_SAMPLE` hoặc `DIAGNOSTIC_SAMPLE`, phải cập nhật canonical enum bằng decision/schema review. MVP không đổi tag ngầm.
...
provenance == PROVENANCE_LIVE_PRODUCTION
source_generation is current
config/profile/calibration versions are compatible
...
sequenceDiagram
    ...
    IRQ-->>MM: EVT_MAX_RESULT_READY
    MM->>MAX: asynchronous coherent read request
    MAX-->>MM: raw result/status completion
...
* PC13/MAX INT adapter chỉ capture source/time và post `EVT_MAX_RESULT_READY`.
...
### 16.3. Implementation file mapping

| Current/target implementation               | Mapping                                                    |
| ------------------------------------------- | ---------------------------------------------------------- |
| `3.firmware/include/core/data_model.h`      | Events, result metadata/types                              |
| `3.firmware/include/core/app_event_queue.h` | Measurement event envelope/priority                        |
| `3.firmware/include/core/scheduler.h`       | Due/timeout jobs                                           |
| `3.firmware/src/core/app_event_loop.c`      | Turn dispatch/final publication; Phase 2 handler extension |
| `3.firmware/src/core/data_repository.c`     | Result acceptance/snapshot publication policy extension    |
| `3.firmware/src/core/mode_guard.c`          | Real readiness/blocker evidence extension                  |
| `3.firmware/src/measurement/*`              | Target stream owners and processing adapters               |
| `3.firmware/src/platform/linux/*`           | Emulator/virtual-time bindings                             |
