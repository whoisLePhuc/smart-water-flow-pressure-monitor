...
| `TemperatureResult` | `RESULT` | Temperature/calibration processing | Processing owner được firmware chốt | Flow compensation, repository |
...
MAX raw temperature-related result
  -> validate device/result status
  -> convert using selected measurement principle
  -> apply calibration if defined
  -> assign quality and time metadata
  -> publish TemperatureResult
```
...
| `NORMAL` | Production measurement/product state | Full runtime snapshot | Checkpoint, BLE config, telemetry |
...
| `OQ-DATA-003` | Temperature processing owner cuối cùng? | `TemperatureResult` API |
