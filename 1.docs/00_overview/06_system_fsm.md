...
|---|---|
| `G_CORE_READY` | Core configuration, measurement path và runtime repositories đủ khả năng vận hành |
...
- Tạm dừng hoặc giới hạn normal scheduler theo policy.
...
|---|---|---|
| `OQ-FSM-001` | Core-ready tối thiểu có bắt buộc flow measurement không? | `INIT -> NORMAL` |
| `OQ-FSM-002` | Có cho `NORMAL` degraded khi flow unavailable không? | Readiness/error policy |
| `OQ-FSM-003` | Service mode có cho phép normal measurement song song không? | Entry/allowed operations |
