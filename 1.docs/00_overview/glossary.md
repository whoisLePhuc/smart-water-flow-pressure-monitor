...
Những khối sau chưa thuộc baseline nếu chưa có quyết định kiến trúc mới:
...
| `Configuration apply` | Quá trình đưa config hợp lệ thành `ActiveConfig` và thông báo service liên quan. | `ConfigRepository` |
...
| `Remote downlink` | Luồng server-to-device command/config; chưa thuộc baseline. |
