...
| `Pressure firmware variant` | -        | Firmware artifact tạo từ codebase chung nhưng bind một pressure sensor/ZSSC3241 profile tuple cụ thể.                                                                                 |
| `ProductVariantManifest`    | -        | Manifest định danh build variant và tập compatible profile/calibration schema; là nguồn kiểm tra compatibility khi boot.                                                              |
| `PressureSensorProfile`     | -        | Versioned build-time definition của sensor model, reference type, bridge/range/limits và đặc tính cần cho pressure processing.                                                        |
| `Zssc3241Profile`           | -        | Versioned build-time cấu hình ZSSC3241 tương thích với `PressureSensorProfile`, gồm capability, register configuration và conversion timing.                                          |
| `PressureCalibrationRecord` | -        | Persistent per-device record chứa sensor binding, zero/gain/temperature correction, version/counter và integrity.                                                                     |
| `PressureRuntimeConfig`     | -        | Tập operational field được phép đổi lúc runtime trong allowlist và bounds của product variant; không chứa sensor identity, bridge topology hoặc generic raw-register write.           |
...
* Dùng `profile` cho model/variant definition, `calibration record` cho correction riêng của thiết bị và `runtime config` cho field vận hành được phép thay đổi; không trộn ba lớp này.
...
| `PressureResult`                     | Áp suất đã validate, filter và calibration; kèm timestamp/quality cùng variant/profile/calibration/runtime-config provenance.                                                                                                                                              | `PressureProcessingService`                   |
...
| `Configuration authority` | Thành phần có quyền quyết định config có hợp lệ và được apply hay không; trong baseline là STM32, không phải nRF52810.                                                             | MCU/config services                   |
...
| `Report sequence`          | Số thứ tự của telemetry record; giữ nguyên cùng timestamp/payload khi retry.                                                    |
...
| `Delivery result`          | Kết quả adapter: MQTT `PUBACK` hoặc HTTP `2xx` là success; timeout/no response là unknown/retryable.                            |
...
| `Acknowledgement`          | Transport response dùng cho MVP: matching MQTT QoS 1 `PUBACK` hoặc HTTP `2xx`; không yêu cầu application-level ACK.             |
...
| `nRF52810 BLE coprocessor` | Thực hiện BLE radio/stack/GATT, custom AT parser và UART transport; không sở hữu product configuration.                                     |
| `BLE UART`                 | Dedicated UART `115200 8N1`, không RTS/CTS giữa STM32 và nRF52810.                                                                          |
...
| `Transparent UART mode`    | Mô hình chuyển byte BLE↔UART gần như trong suốt; không phải baseline của nRF52810 trong MVP.                                                |
| `BLE AT control plane`     | Tập lệnh AT do dự án định nghĩa để STM32 điều khiển nRF52810; syntax/framing/version thuộc communication contract riêng.                    |
| `BLE application frame`    | Bản tin ứng dụng tự định nghĩa giữa điện thoại và STM32, được vận chuyển qua custom GATT và UART; schema sẽ được chốt trong tài liệu riêng. |
...
| Thuật ngữ                  | Định nghĩa                                                                                                                                                                 |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `EC200U-CN`                | Quectel LTE Cat 1 bis modem dùng AT và TCP/IP stack bên trong modem; PPP trên STM32 không thuộc MVP.                                                                       |
| `Cellular UART`            | Dedicated UART `115200 8N1` có RTS/CTS giữa STM32 và EC200U-CN.                                                                                                            |
...
| `Network registration`     | Quá trình EC200U-CN đăng ký vào mạng di động.                                                                                                                              |
| `Data connection`          | Kết nối packet-data cho application protocol.                                                                                                                              |
| `Modem AT transaction`     | Một command/response có timeout và owner rõ ràng giữa STM32 và EC200U-CN.                                                                                                  |
| `URC`                      | Unsolicited Result Code do modem tự phát sinh, được parser tách khỏi command response.                                                                                     |
| `RTS/CTS`                  | Hardware flow control dùng trên UART EC200U-CN; không dùng trên UART nRF52810 trong MVP.                                                                                   |
...
| `TelemetryTransport`       | Common interface che `MqttTransportAdapter` và `HttpTransportAdapter`; mỗi profile chọn một active transport.                                                              |
| `Application protocol`     | MQTT QoS 1 hoặc HTTP POST với cùng versioned JSON envelope trong MVP.                                                                                                      |
| `PUBACK`                   | MQTT QoS 1 acknowledgement từ broker cho matching publish packet identifier.                                                                                               |
| `HTTP 2xx`                 | HTTP success response được MVP dùng làm terminal transport response.                                                                                                       |
...
Không gọi `EC200U-CN` là `server`. Modem chỉ cung cấp connectivity/transport; server là endpoint bên ngoài.
...
| `HTTP`  | Hypertext Transfer Protocol                 | MVP dùng HTTP POST adapter                                         |
...
| `MQTT`  | Message Queuing Telemetry Transport         | MVP dùng publish QoS 1 và PUBACK                                   |
...
| `OQ-TERM-005` | Server protocol và delivery acknowledgement                                                                                                                              | Resolved by `DEC-COM-001/002`: MQTT QoS 1 hoặc HTTP POST; `PUBACK`/HTTP `2xx` |
