...
### 7.2. Đề xuất phải review trước firmware implication

| Decision ID | Chủ đề | Status | Gate |
|---|---|---|---|
| `DEC-ARCH-001` | Flow path và core readiness | `PROPOSED` | `GATE-A` |
| `DEC-ARCH-002` | Owner của temperature processing | `PROPOSED` | `GATE-A` |
| `DEC-ARCH-003` | Uncompensated production flow | `PROPOSED` | `GATE-A` |
| `DEC-ARCH-004` | Production measurement trong `SERVICE` | `PROPOSED` | `GATE-A` |
| `DEC-ARCH-005` | Logical I2C bus manager | `PROPOSED` | `GATE-A` |
| `DEC-ARCH-006` | Snapshot publication strategy | `PROPOSED` | `GATE-A` |
| `DEC-ARCH-007` | Config apply acknowledgement | `PROPOSED` | `GATE-A` |
| `DEC-ARCH-008` | OTA scope | `PROPOSED` | `GATE-A` |
...
## 9. Kiến trúc cần quyết định trước tài liệu 11
...
| Proposed decision | Trong một production boot, `CORE_MEASUREMENT_READY` chỉ đạt khi MAX35103/flow path đã khởi tạo thành công và tạo được ít nhất một kết quả kiểm tra hoặc measurement hợp lệ theo readiness policy. Nếu chưa đạt, hệ thống không chuyển `INIT -> NORMAL`; nó thực hiện bounded initialization recovery và sau đó chuyển `RECOVERY`, `SERVICE` hoặc `ERROR` theo kết quả. Sau khi hệ thống đã vào `NORMAL`, một flow fault tạm thời chỉ làm MeasurementStatus chuyển `DEGRADED`; volume và leak evidence phụ thuộc flow bị tạm dừng trong khi các subsystem không liên quan tiếp tục hoạt động. Nếu local recovery hết giới hạn, hệ thống chuyển sang `RECOVERY`; chỉ chuyển `ERROR` khi coordinated recovery thất bại và không có degraded-safe operation được chấp nhận. |
...
| Proposed decision | Temperature conversion/calibration thuộc `CalibrationService`; `TemperatureResult` vẫn là data object độc lập được publish. |
...
| Proposed decision | Không cho uncompensated production flow trong baseline. Temperature không phù hợp làm flow result `INVALID` hoặc `DEGRADED_NOT_ACCEPTED`; chỉ mở fallback sau khi validation chứng minh sai số chấp nhận được. |
| Rationale | Ngăn volume/leak sử dụng flow có sai số chưa định lượng. |
| Source OQ | 03:OQ-OP-011, 04:OQ-FLOW-010, 08:OQ-DATA-004 |
| Affected docs | 03, 04, 05, 08, 09, 11 |
...
| Proposed decision | Production measurement tạm dừng mặc định. Service profile có thể chạy `SERVICE_SAMPLE`/`CALIBRATION_SAMPLE`; các sample này không update production volume/leak/telemetry. |
| Rationale | Đơn giản hóa scheduling và ngăn test stimulus làm nhiễm product state. |
| Source OQ | 06:OQ-FSM-003, 07:OQ-MODE-005 |
| Affected docs | 06, 07, 08, 11 |
...
| Proposed decision | Dùng logical `I2cBusManager`/bus-owner abstraction cho arbitration, timeout và recovery. Mỗi physical I2C instance có một owner context; ZSSC3241 và F-RAM có thể map cùng hoặc khác instance sau. |
...
| Affected docs | 04, 08, 09, 10, 11, hardware docs |
...
| Proposed decision | Dùng double buffer; writer cập nhật inactive buffer rồi atomic swap active index. |
...
| Proposed decision | `ConfigRepository` gửi `config_version` tới service đích; service apply tại safe boundary và trả `APPLIED`, `DEFERRED` hoặc `REJECTED` cùng reason/version. |
| Rationale | Phân biệt persistent commit thành công với runtime apply thành công. |
| Source OQ | 05:OQ-SEQ-004 |
| Affected docs | 04, 05, 08, 11 |
...
| Proposed decision | OTA và remote configuration qua 4G không thuộc baseline hiện tại; là future scope cần system architecture review riêng. |
| Rationale | Chưa có bootloader, image storage, rollback, signing và security requirement. |
| Source OQ | 01:OQ-011, 03:OQ-OP-013 |
| Affected docs | README, 01, 02, 10, 11 |
...
| `DEC-PWR-002` | Critical power: `ERROR` hay shutdown flow | `OPEN` | `GATE-A` | 06:OQ-FSM-010, 07:OQ-MODE-011, 09:OQ-ERR-011 | Cần architecture decision trước final FSM mapping |
...
`DEC-PWR-002` là architecture blocker còn mở ngoài tám đề xuất chính. Nếu power hardware chưa đủ thông tin, tài liệu 11 phải mô hình hóa một `PowerCriticalAction` policy interface và ghi rõ final FSM path chưa chốt.
...
Riêng `DEC-PWR-002` có thể ở `OPEN` nếu tài liệu 11 không thêm dedicated shutdown state và giữ rõ `PowerCriticalAction` là policy point.
...
| `DEC-ARCH-003` | Có dữ liệu validation cho phép uncompensated flow không? |
| `DEC-ARCH-004` | Service profile nào thực sự cần production measurement song song? |
| `DEC-ARCH-005` | Abstraction có hỗ trợ cả một và hai physical I2C instance không? |
| `DEC-ARCH-006` | RAM budget có đủ cho hai snapshot buffer không? |
| `DEC-ARCH-007` | Service nào cần `DEFERRED` apply và timeout bao lâu? |
| `DEC-ARCH-008` | Có requirement OTA/remote config chính thức không? |
| `DEC-PWR-002` | Hardware có controlled shutdown hay chỉ reset/brownout protection? |
...
```text
DECIDED system baselines : 8
PROPOSED GATE-A items    : 8
Additional power GATE-A : 1 open policy point
...
1. Review `DEC-ARCH-001` đến `DEC-ARCH-008`.
2. Xác định cách xử lý tạm thời cho `DEC-PWR-002`.
3. Cập nhật trạng thái `DECIDED` cho các đề xuất được chấp nhận.
4. Hiệu chỉnh OQ nguồn trong tài liệu 01–10.
5. Triển khai `11_firmware_implication.md`.
