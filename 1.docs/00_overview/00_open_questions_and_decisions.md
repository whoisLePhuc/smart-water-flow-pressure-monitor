...
| Proposed decision | Flow là core measurement. Boot không vào `NORMAL` nếu flow path chưa từng đạt readiness. Trong runtime, flow fault tạm thời được phép `NORMAL` degraded trong bounded local recovery; hết recovery budget thì vào `RECOVERY`, sau đó có thể `ERROR`. |
