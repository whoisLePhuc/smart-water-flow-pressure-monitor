...
| Proposed decision | Trong production boot, flow path phải đạt `CORE_MEASUREMENT_READY` trước khi hệ thống chuyển `INIT -> NORMAL`. Trong runtime, flow fault tạm thời được phép giữ `SystemMode=NORMAL` với `MeasurementStatus=DEGRADED` trong thời gian bounded local recovery. Khi local recovery hết giới hạn, hệ thống chuyển `RECOVERY`. Nếu coordinated recovery thất bại và không còn core flow operation an toàn, hệ thống chuyển ERROR.|
