...
**Revision marker:** `2026-07-14 — shared I2C and persistent-storage policy approval`

---

> **Decision update:** `DEC-HW-006` chốt ZSSC3241 và FM24CL04B dùng chung một physical I2C dưới quyền `I2cBusManager`. `DEC-DATA-001`, `DEC-DATA-004` và `DEC-DATA-005` chốt configurable volume-checkpoint policy, fixed F-RAM A/B map và storage-admission policy theo record class. Checkpoint hiện có 34 decision đã chốt.
...
Tài liệu 13 đã tạo policy model và decision package. `DEC-SCHED-001`–`DEC-SCHED-004` đã được phê duyệt; các communication decision còn lại giữ `OPEN/DEFERRED` cho tới khi owner phê duyệt.
