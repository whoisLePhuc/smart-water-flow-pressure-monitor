...
**Short name:** SWFPM
**Status:** Traceability baseline with implementation checkpoint `9c654b6`
**Language:** Vietnamese; canonical identifier và verification code có thể dùng tiếng Anh

---

## 0. Implementation evidence checkpoint

Checkpoint code: `9c654b6467e3f86f8f71c76e04feedcfca427c65`. Traceability không còn ở trạng thái “chỉ DEFINED/PLANNED” tuyệt đối: đã có implementation reference cho composition/event core, FSM/guard, scheduler, repository transaction/snapshot, measurement registry, processing, power, storage foundation, reporting policy/queue và Linux simulation. Tuy vậy, `IMPLEMENTED` không đồng nghĩa `VERIFIED`; không nâng requirement lên `VERIFIED` nếu chưa có evidence test được review.

MAX/ZSSC production acquisition, compute binding, canonical storage port, full STM32 adapters, BLE/modem/LCD, health/watchdog và diagnostic log tiếp tục **Partial/Planned**. CMake/CTest không được ghi nhận là đã chạy trong checkpoint tài liệu này vì môi trường review không có CMake binary.
...
* Các `DEC-SYS-*`, `DEC-ARCH-*`, `DEC-PWR-002`, `DEC-HW-001/002/003/006/007`, `DEC-DATA-001`–`DEC-DATA-005`, `DEC-ERR-001`–`DEC-ERR-005`, `DEC-MODE-001`, `DEC-SVC-001` và `DEC-COM-001`–`DEC-COM-004` đã chốt.
...
Baseline hiện tại cho phép dùng `IMPLEMENTED` khi có code reference cụ thể, nhưng không tự động suy ra `VERIFIED`. Mọi trạng thái `VERIFIED` vẫn cần evidence được review và gắn đúng build/commit.
...
| `DEC-ERR-005` (`DECIDED`), `DEC-DIAG-001`                  | `REQ-FW-008`, `REQ-FW-015`                                                                         | Numeric registry/assertion closed; diagnostic retention/upload remains open                                                           |
...
| `DEC-SVC-001`, `DEC-MODE-001` (`DECIDED`)                  | `REQ-FW-029`–`REQ-FW-030`, `REQ-FW-058`, `REQ-FW-066`, `REQ-FW-070`, `REQ-FW-073`                  | Limited INIT BLE and role-based bounded SERVICE closed; detailed pairing/key provisioning remains                                     |
...
| `TG-006` | Detailed BLE pairing/key provisioning chưa chốt                                       | System role/allowlist/clear policy đã chốt; production security exposure vẫn cần contract                  | Communication/security docs          |
| `TG-007` | Diagnostic retention/coalescing/upload chưa chốt                                      | Numeric error/assertion policy đã chốt; retention vẫn `OPEN`                                               | `DEC-DIAG-001`                       |
...
* [x] 48 decided baseline có downstream mapping.
