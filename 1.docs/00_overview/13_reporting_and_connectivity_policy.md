...
* Mỗi start time phải nằm trong một local-day hợp lệ.
* Mỗi interval phải dương và nằm trong min/max sau khi `DEC-SCHED-004` được chốt.
...
Schedule được đánh giá trên local time do `TimeService` cung cấp. Exact timezone model còn TBD:

* Fixed UTC offset; hoặc
* Timezone rule có daylight-saving support.

Khuyến nghị ban đầu là fixed offset versioned để giảm ambiguity trên MCU, nhưng cần được duyệt cùng deployment region.
...
`DEC-SCHED-003` vẫn `OPEN`.

**Current baseline:** leak state change cập nhật `RuntimeSnapshot`/diagnostics; không tự tạo immediate production telemetry.

**Proposed decision:** giữ scheduled-only cho MVP. Nếu sau này cần immediate leak report:
...
| `REQ-RCP-020` | Baseline telemetry trigger là scheduled report; immediate leak telemetry không được enable trước `DEC-SCHED-003`. |
...
| `REQ-RCP-030` | Non-scheduled record tương lai phải có distinct reason/dedup policy và không giả scheduled slot.                  |
...
| `DEC-SCHED-004` | 8, 21, 25      | `REQ-RCP-007`, `REQ-RCP-018`                                             |
