...
Exact nRF52810 custom-AT and EC200U-CN recovery command sequence
...
| `BLE`                    | nRF52810, UART/custom AT, application frame, session, authorization     |
| `CELLULAR`               | EC200U-CN, UART/RTS-CTS, AT response/URC, registration, data connection |
...
| Temperature invalid/unusable | Mark temperature invalid; flow `INVALID` hoặc `DEGRADED_NOT_ACCEPTED` | Bounded retry/re-read/re-measure | Persistent loss of compensation → flow recovery escalation |
...
Theo `DEC-ARCH-001`, production boot không vào `NORMAL` nếu flow path chưa tạo được valid readiness evidence. Trong runtime, flow fault tạm thời giữ `NORMAL + DEGRADED` trong bounded local recovery; hết local budget thì yêu cầu system recovery. Chỉ failed coordinated recovery và không còn core flow operation an toàn mới dẫn tới `ERROR`.

Theo `DEC-ARCH-003`, firmware không được biến uncompensated flow thành production flow trong baseline. Result bị từ chối vẫn có thể xuất hiện trong repository/diagnostics với reason rõ ràng, nhưng không update volume, không tạo flow-based leak evidence và không được báo cáo như valid production measurement. Held-temperature hoặc uncompensated fallback chỉ được mở sau validation và bằng versioned policy.
...
| ------------------------------------- | ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| I2C NACK/timeout                      | Mark sample invalid; correlate failed transaction                  | `I2cBusManager` bounded retry/abort; device owner re-init nếu cần                                     | Flow measurement tiếp tục                                                  |
| Device status fault                   | Reject raw result                                                  | Reconfigure/self-check                                                                                | Pressure unavailable/degraded                                              |
| Manifest/profile/calibration mismatch | Chặn production acceptance; publish configuration/integrity reason | Không tự chọn fallback profile; yêu cầu đúng firmware variant hoặc authorized reprovision/calibration | Pressure unavailable; diagnostics/service vẫn có thể hoạt động theo policy |
...
| Shared I2C stuck                      | `I2cBusManager` stop admission cho instance bị ảnh hưởng           | Owner-context coordinated bus recovery và increment recovery generation                               | System `RECOVERY` nếu nhiều service bị ảnh hưởng                           |

Pressure unavailable không được thay bằng zero và không tự động xác nhận/xóa leak state.

Theo `DEC-ARCH-005`, `PressureMeasurementService`, `StorageService` và device driver không được tự reset/reinitialize shared I2C peripheral. Mỗi physical instance có đúng một `I2cBusManager` owner. Manager phải:

* Chặn transaction admission mới trước recovery.
* Hoàn tất/cancel in-flight request bằng kết quả xác định.
* Thực hiện timeout và bounded recovery trong owner context.
...
| Calibration invalid       | Không dùng calibration giả; `DEC-HW-001` yêu cầu matching sensor binding/schema/CRC và chặn accepted production pressure |
...
### 27.2. EC200U-CN modem/UART fault
...
EC200U-CN fault chỉ yêu cầu system recovery nếu làm hỏng shared power/UART/platform resource hoặc local modem recovery gây ảnh hưởng đa subsystem. Command response và URC phải được parse tách biệt; timeout/recovery phải bounded. Offline kéo dài một mình không phải `SystemMode.ERROR`.
...
| MAX event clock unavailable   | Production flow path bị degrade và đi qua bounded recovery; direct mode chỉ được phép trong service/calibration/diagnostic context và không được tạo production side effect |
| ZSSC3241 EOC/result timeout   | Hủy request generation, release I2C ownership, publish pressure unavailable/degraded và chạy bounded local recovery; flow path tiếp tục                                     |
...
* MVP không tạo immediate critical/leak event telemetry; fault state được đưa vào scheduled telemetry kế tiếp. Event telemetry trong tương lai cần policy/security/power review riêng.
...
| Pressure/ZSSC/I2C    | `I2cBusManager` + pressure device owner | Recovery coordinator                                 |
| Storage/F-RAM I2C    | `I2cBusManager` + `StorageService`      | Recovery coordinator/SystemModeManager               |
...
* Temperature unusable tạo `INVALID`/`DEGRADED_NOT_ACCEPTED` và không đi vào production consumers.
* Uncompensated diagnostic value không được serialize như valid production telemetry.
...
production boot without valid flow readiness remains INIT
runtime flow fault remains NORMAL + DEGRADED within local budget
local flow budget exhausted enters RECOVERY
failed coordinated flow recovery enters ERROR
...
| `REQ-ERR-033` | Production boot không được vào `NORMAL` khi flow path chưa có valid readiness evidence trong boot session hiện tại.                              |
| `REQ-ERR-034` | Runtime flow fault tạm thời phải giữ `NORMAL + DEGRADED` trong bounded local recovery và không update volume/valid flow evidence.                |
| `REQ-ERR-035` | Hết local flow recovery budget phải yêu cầu system recovery; chỉ failed coordinated recovery không còn safe core flow operation mới vào `ERROR`. |
| `REQ-ERR-036` | Temperature compensation không khả dụng phải làm flow result `INVALID` hoặc `DEGRADED_NOT_ACCEPTED`.                                             |
| `REQ-ERR-037` | Flow bị từ chối do temperature không được update volume, tạo valid flow-based leak evidence hoặc trở thành valid production telemetry.           |
| `REQ-ERR-038` | Uncompensated/held-temperature fallback chỉ được enable khi có validation evidence và versioned bounded policy.                                  |
| `REQ-ERR-039` | Mỗi physical I2C instance phải có đúng một `I2cBusManager` recovery owner.                                                                       |
