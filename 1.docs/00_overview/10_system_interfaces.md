...
**Status:** Interface baseline aligned with firmware `9c654b6`

---

## 0. Implementation alignment

Các interface đã hiện diện trong code gồm event queue/router, repository transaction/snapshot, `AdcPort`, `StoragePort`, monotonic/system-control port, facade và Linux providers. `StorageService` chưa dùng `StoragePort` làm boundary canonical mà vẫn giữ `FramDriver*`. STM32 `AdcPort` là polling đồng bộ; full SPI/I2C/UART/BLE/modem/LCD adapter chưa hoàn chỉnh. `PressureMeasurementService` trong bảng logical interface tương ứng với ZSSC measurement entry do `MeasurementManager` sở hữu.
...
| Owner transaction I2C | Shared `I2cBusManager` cho ZSSC3241 và FM24CL04B                                                           |
...
* Persistent record dùng fixed per-type A/B map, explicit serialization, version, sequence và CRC32.
* Pressure transaction có priority cao hơn storage; arbitration không preempt transaction đang active.
* `StorageService` giữ một immutable commit in-flight và admission theo record class; volume pending là latest-wins.
* FM24CL04B không dùng làm persistent telemetry queue trong MVP.
...
`LIF-14` bind ZSSC3241 và FM24CL04B vào cùng physical I2C theo `DEC-HW-006`. Bus có đúng một `I2cBusManager`; pressure transaction được ưu tiên hơn storage và service contract không expose HAL/recovery.
...
| Driver, HAL, DMA, ISR và internal FSM            | `../05_firmware/`                         |
