...
- Remote configuration qua 4G chưa thuộc baseline nếu chưa có quyết định mới.
```

Các model cụ thể của pressure sensor, BLE module, 4G module và LCD sẽ được chốt trong tài liệu hardware. Việc chưa chọn model không làm thay đổi ranh giới chức năng được định nghĩa trong tài liệu này.
...
Cảm biến áp suất giao tiếp với MCU qua I2C. Dữ liệu áp suất phải đi qua kiểm tra trạng thái, range check, filtering và calibration trước khi được publish.
...
ReportingWindow[0]           : Configurable start time and interval
ReportingWindow[1]           : Configurable start time and interval
Initial default/example      : ReportingWindow[0] = 15 minutes, ReportingWindow[1] = 5 minutes
```
...
Sau khi configuration được validate và persistent commit thành công, `ReportingScheduler` áp dụng `ActiveConfig` mới và tính lại `next_report_time`. Việc thay đổi schedule không hủy telemetry transaction đang chạy và không tự động tạo report ngay lập tức.
...
| Pressure sensor | Đo áp suất nước | Raw pressure, sensor status |
...
flowchart TD
    ...
    PRESS["Pressure sensor measurement"] --> PPROC["Pressure processing"]
...
|---|---|---|
| `OQ-001` | Model và dải đo của pressure sensor | Hardware interface, calibration, validation và test |
...
| `OQ-005` | Default start time và giới hạn interval hợp lệ của `ReportingWindow[0]`/`ReportingWindow[1]` là gì? | ReportingSchedule default/validation |
| `OQ-006` | Tiêu chí và threshold xác nhận rò rỉ | LeakDetectionService và acceptance test |
...
| `OQ-010` | Nguồn đồng bộ thời gian ưu tiên: BLE, 4G network hoặc server | TimeService và RTC validity policy |
| `OQ-011` | Có hỗ trợ OTA hay không | Storage, bootloader, security và communication architecture |
