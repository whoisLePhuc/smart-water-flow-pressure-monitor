...
| `CORE_MEASUREMENT_READY` | Measurement path cốt lõi có thể chạy theo product policy |
...
5. Tạm dừng hoặc giới hạn normal scheduler theo profile.
...
| Measurement | `LIMITED` theo service profile |
| Volume accumulation | `QUIESCED` mặc định khi dùng test stimulus |
| Leak detection | `QUIESCED` hoặc `LIMITED`; test data không được tạo product alarm |
| Reporting scheduler | `LIMITED` hoặc `QUIESCED` theo profile |
...
6. Đánh giá runtime readiness.
7. Close session và record exit reason.
8. Chuyển `NORMAL`, `RECOVERY` hoặc `ERROR` qua FSM.
...
|---|---|---|
| `OQ-MODE-001` | Core readiness có bắt buộc flow path available không? | `INIT -> NORMAL` |
| `OQ-MODE-002` | Có cho `NORMAL` degraded khi flow unavailable không? | Normal/error boundary |
...
| `OQ-MODE-005` | Production measurement có tiếp tục trong từng service profile không? | Data isolation/scheduling |
