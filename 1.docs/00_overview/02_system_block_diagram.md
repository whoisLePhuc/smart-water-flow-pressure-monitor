...
flowchart TD
    ...
    PRESSDATA["Pressure sensor result"] --> PMEAS["PressureMeasurementService"]
...
Pressure sensor đo áp suất nước và giao tiếp với MCU qua I2C.

Vai trò chính:

- Cung cấp raw pressure measurement.
- Cung cấp sensor status hoặc diagnostic nếu phần cứng hỗ trợ.
- Bổ sung dữ liệu cho monitoring và leak detection policy.

Model, dải đo, độ chính xác, sample rate và calibration requirement hiện là `TBD`.
...
Remote configuration từ server qua 4G chưa thuộc baseline hiện tại nếu chưa có architecture decision mới.
...
| Pressure measurement | MCU ↔ pressure sensor | I2C | Cấu hình và đọc pressure/status |
...
|---|---|---|
| `OQ-001` | Pressure sensor model, range và accuracy | Pressure sensing and processing |
