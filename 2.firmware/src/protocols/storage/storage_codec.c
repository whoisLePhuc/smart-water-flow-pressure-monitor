...
static uint32_t volume_record_crc(const uint8_t *buf)
{
    uint32_t crc = crc32_partial(0xFFFFFFFFu, buf, 12u);
    crc = crc32_partial(crc, buf + HDR_SIZE, PAYLOAD_SIZE);
...
uint16_t storage_codec_encode_volume(uint8_t *buf, const VolumeStoragePayload *p)
{
    ...
    uint32_t crc = volume_record_crc(buf);
...
bool storage_codec_decode_volume(const uint8_t *buf, VolumeStoragePayload *p)
{
    ...
    if (buf[6] != PAYLOAD_SIZE || buf[7] != 0u) return false;

    uint32_t stored_crc = (uint32_t)buf[12]
        | ((uint32_t)buf[13] << 8)
        | ((uint32_t)buf[14] << 16)
        | ((uint32_t)buf[15] << 24);
    if (volume_record_crc(buf) != stored_crc) return false;
