...
#include "simulation/stubs/processing_stubs.h"
#include <stdio.h>
#include <assert.h>
#include <string.h>
...
static void test_service_result_not_production(void)
{
    ...
    assert(!result_metadata_is_production(&p.meta));
    PASS();
}
...
static void test_production_eligibility(void)
{
    ...
    ResultMetadata valid = make_source_meta(
        ...
        DATA_ORIGIN_LIVE_DEVICE,
        PROVENANCE_MEASURED);
    assert(result_metadata_is_production(&valid));
    ...
    ResultMetadata no_live = make_source_meta(
        ...
        DATA_ORIGIN_SIMULATED_DEVICE,
        PROVENANCE_MEASURED);
    assert(!result_metadata_is_production(&no_live));
    ...
    assert(!result_metadata_is_production(&estimated));
    ...
    ResultMetadata service = make_source_meta(
        ...
        DATA_ORIGIN_LIVE_DEVICE,
        PROVENANCE_MEASURED);
    assert(!result_metadata_is_production(&service));
    PASS();
}
