...
static void test_app_loop_context_is_instance_owned(void)
{
    ...
    scheduler_init(&scheduler_a);
    scheduler_init(&scheduler_b);
    app_event_loop_init(&loop_a, &queue_a, &fsm_a, &repo_a,
                        &mediator_a, &scheduler_a, NULL);
    app_event_loop_init(&loop_b, &queue_b, &fsm_b, &repo_b,
                        &mediator_b, &scheduler_b, NULL);
